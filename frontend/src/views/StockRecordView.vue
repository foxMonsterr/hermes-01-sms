    ...
    <el-tabs v-model="activeTab">
      <el-tab-pane label="库存流水" name="records">
        ...
        <el-table :data="records" stripe v-loading="loading" empty-text="暂无流水">
          ...
          <el-table-column prop="changeType" label="类型" width="90"><template #default="{row}"><el-tag :type="typeTag(row.changeType)" size="small">{{typeLabel(row.changeType)}}</el-tag></template></el-table-column>
      ...
      <el-tab-pane label="盘点记录" name="checks">
        <div class="search-bar">
          <el-date-picker v-model="checkDateRange" type="daterange" range-separator="至" value-format="YYYY-MM-DD" style="width:260px" @change="fetchChecks" />
          <el-button type="primary" @click="fetchChecks">查询</el-button>
        </div>
        <div class="stats-cards" v-if="checkStats">
          <span>总盘点: {{checkStats.totalCheckCount}} | 有差异: {{checkStats.differenceCount}} | 盘盈: +{{checkStats.totalPositiveDiff}} | 盘亏: -{{checkStats.totalNegativeDiff}}</span>
        </div>
        <el-table :data="checks" stripe v-loading="checkLoading" empty-text="暂无盘点记录">
          <el-table-column prop="snackName" label="零食" min-width="120" />
          <el-table-column prop="systemQty" label="系统库存" width="90" />
...
// records
const records = ref<StockRecord[]>([]); const total = ref(0); const loading = ref(false)
...
// checks
const checks = ref<InventoryCheckRecord[]>([]); const checkTotal = ref(0); const checkLoading = ref(false)
const checkPage = ref(1); const checkSize = ref(10)
const checkDateRange = ref<[string,string]|null>(null)
const checkStats = ref<InventoryCheckStats|null>(null)
...
<style scoped>.page{background:#fff;border-radius:8px;padding:20px}.search-bar{display:flex;gap:12px;margin-bottom:16px}.pagination{margin-top:16px;display:flex;justify-content:flex-end}.stats-cards{margin-bottom:12px;padding:8px 12px;background:#f5f7fa;border-radius:4px;font-size:13px;color:#606266}</style>
