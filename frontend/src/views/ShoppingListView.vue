<template>
  <div class="page">
    ...
    <el-table :data="items" stripe v-loading="loading" empty-text="暂无清单">
      <el-table-column prop="snackName" label="名称" min-width="140" />
      <el-table-column prop="categoryName" label="分类" width="100" />
      <el-table-column prop="plannedQty" label="计划数量" width="90" />
      <el-table-column prop="price" label="预估单价" width="90" />
    ...
    <!-- 新增/编辑弹窗 -->
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑' : '新增采购项'" width="420px">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="80px">
        <el-form-item label="名称" prop="snackName"><el-input v-model="form.snackName" maxlength="100" /></el-form-item>
        <el-form-item label="计划数量" prop="plannedQty"><el-input-number v-model="form.plannedQty" :min="1" /></el-form-item>
        <el-form-item label="预估单价"><el-input v-model="form.price" placeholder="选填" /></el-form-item>
        <el-form-item label="供应商">
          <el-select v-model="form.supplierId" placeholder="选择供应商" clearable filterable style="width:100%">
    ...
    <el-dialog v-model="stockInVisible" title="采购入库" width="380px">
      <el-form label-width="80px">
        <el-form-item label="零食"><span>{{ stockInItem?.snackName }}</span></el-form-item>
        <el-form-item label="实际数量"><el-input-number v-model="stockInForm.actualQty" :min="1" /></el-form-item>
        <el-form-item label="实际单价"><el-input v-model="stockInForm.price" placeholder="选填" /></el-form-item>
        <el-form-item label="备注"><el-input v-model="stockInForm.remark" /></el-form-item>
      </el-form>
      <template #footer>
...
const stockInItem = ref<ShoppingListItem | null>(null)
...
async function fetchData() {
  loading.value = true
  try {
    const res = await getShoppingList(page.value, size.value, activeTab.value || undefined, keyword.value || undefined)
...
async function handleSubmit() {
  ...
  try {
    const data: any = { snackName: form.snackName, plannedQty: form.plannedQty, price: form.price ? Number(form.price) : undefined, remark: form.remark, supplierId: form.supplierId }
    if (editingId.value) { await updateShoppingItem(editingId.value, data) } else { await createShoppingItem(data) }
...
async function handleFromLowStock() {
  const res = await fromLowStock()
  ElMessage.success(`已生成 ${res.data.createdCount} 条，跳过 ${res.data.skippedCount} 条`)
  fetchData()
}
...
async function handleStockIn() {
  ...
  try {
    const res = await stockIn(stockInItem.value!.id, { actualQty: stockInForm.actualQty, price: stockInForm.price ? Number(stockInForm.price) : undefined, remark: stockInForm.remark })
    ElMessage.success(`入库成功，当前库存: ${res.data.quantity}`)
